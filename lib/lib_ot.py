...
@nb.njit(cache=True)
def ot1d(u_weights,v_weights):
    # need to sort u_values, v_values first 
    N,M=u_weights.shape[0],v_weights.shape[0]
    gamma=np.zeros((N,M))
    i,j=0,0
    u_mass,v_mass=u_weights[0],v_weights[0]
    while i<N and j<M:
...
@nb.njit(cache=True)
def sinkhorn_knopp(mu, nu, M, reg=0.1, numItermax=1000,verbose=True):
    r"""
    ...
    for ii in range(numItermax):
        ...
        if ii % 10 == 0:
            ...
            if err < stopThr:
                break
    if ii==numItermax-1 and verbose==True:
...
@nb.njit(cache=True)
def sinkhorn_knopp_opt_1(mu,nu,M,Lambda,reg=0.1,numItermax=1000,verbose=False):
    """
    Solves the partial optimal transport problem
    and returns the OT plan by linear programming in PythonOT 
    ...
    # plan1, cost1, u, v, result_code = emd_c(mu1, nu1, M1, numItermax, numThreads)
    # result_code_string = check_result(result_code)
    plan1=sinkhorn_knopp(mu1, nu1, M1, reg, numItermax=numItermax,verbose=verbose)
    plan=plan1[0:n,0:m]
    # cost=np.sum(M*plan)
    return plan
...
# @nb.njit(cache=True)
def sinkhorn_knopp_opt_pr_1(mu, nu, M, mass, **kwargs):
    """
    Solves the partial optimal transport problem for the quadratic cost
    and returns the OT plan
    ...
    **kwargs : dict
    ...
    gamma1=sinkhorn_knopp(mu1, nu1, M1,**kwargs )
    gamma=gamma1[0:n,0:m]
    # cost=np.sum(M*gamma)
    return gamma
...
def opt1d(X,Y,lam): #,verbose=False):
    n,m=X.shape[0],Y.shape[0]
...
@nb.njit(cache=True,fastmath=True)
def opt1d_1(X,Y,lam,dtype=np.float64): #,verbose=False):
    dtype=X.dtype
    n,m=X.shape[0],Y.shape[0]
    phi=np.full(shape=n,fill_value=-np.inf,dtype=dtype)
    psi=np.full(shape=m,fill_value=lam,dtype=dtype)
    # to which cols/rows are rows/cols currently assigned? -1: unassigned
    piRow=np.full(n,-1,dtype=np.int64)
    piCol=np.full(m,-1,dtype=np.int64)
    # a bit shifted from notes. K is index of the row that we are currently processing
